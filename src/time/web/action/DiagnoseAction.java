	...
	public String diagnose() throws DocumentException, IOException, InterruptedException {
		...
		//修改的
				...
				if(typeName.equals("intestineDetection")) {
					...
					String imageTr = "/data/leichao/diagnose/testSe/process/imagesTs/"+user.getLoginname();
					String se_result = "/data/leichao/diagnose/testSe/result/"+user.getLoginname();
					FileUtils.delete(imageTr);
					FileUtils.delete(se_result);
					System.out.println("imageTr::"+imageTr+"---"+"se_result::"+se_result);
...
//这里需要修改
	public String getResultNii() {
		User user = (User) request.getSession().getAttribute("user");
		//String niiName = (String) request.getSession().getAttribute("typeName");
		//System.out.println("niiName::"+niiName);
		String zipName = "test_test.nii.gz";
		response.setContentType(ServletActionContext.getServletContext().getMimeType(zipName));		
		response.setHeader("Content-Disposition", "attachment;filename="+ zipName);
		DiagnoseService diagnose = new DiagnoseServiceImp();
		
		String se_input ="/data/leichao/diagnose/testSe/result"+File.separator+user.getLoginname()+File.separator+zipName;
		System.out.println("se_input::"+se_input);
		diagnose.getPDF(se_input,response);
		return null;
	}
	public String scanDcm() throws ServletException, IOException {	
		User user = (User) request.getSession().getAttribute("user");
		String typeName = (String) request.getSession().getAttribute("typeName");
		String resultDir = "/data/leichao/diagnose/result/"+typeName+"/" + user.getLoginname();
		DiagnoseService diagnose = new DiagnoseServiceImp();
	...
	public String pictureTransport() throws IOException {
		...
		FileInputStream fileInputStream = new FileInputStream("/data/leichao/diagnose/result/"+typeName+"/"+ user.getLoginname()+ "/" + pictureName);
